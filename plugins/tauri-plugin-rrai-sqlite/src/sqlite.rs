...
pub async fn open(state: State<'_, SqliteMap>, path: String) -> Result<bool> {
    //到当前用户的目录的.rrai路径
    let mut storage_path = rrai_desktop_sdk_common::utils::rrai_home_path()?.join("sqlite");
...
#[command]
pub async fn close(state: State<'_, SqliteMap>, path: String) -> Result<bool> {
...
#[command]
pub async fn execute_sql(state: State<'_, SqliteMap>, path: String, sql: String) -> Result<usize> {
    let mut map = state.0.lock().unwrap();
...
#[command]
pub async fn execute_batch(state: State<'_, SqliteMap>, path: String, sql: String) -> Result<bool> {
    let mut map = state.0.lock().unwrap();
    let connection = map.get_mut(&path).ok_or(Error::DatabaseNotOpened(path))?;
