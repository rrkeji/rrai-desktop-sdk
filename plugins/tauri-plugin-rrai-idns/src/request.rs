...
use mpart_async::{
    client::{ByteStream, MultipartRequest},
...
pub async fn rrai_cloud_post_multipart(
    path: &String,
    token: &String,
    request_form: MultipartRequest<ByteStream>,
) -> Result<String> {
    //
    let https = HttpsConnector::new();
    //构建Https Client
    ...
    let request = Request::builder()
        ...
        .header(
            ...
            format!(
                ...
                request_form.get_boundary()
            ),
        )
        .body(Body::wrap_stream(request_form))
        .map_err(|err| {
            tracing::error!("调用{}接口封装request:{:?}", url, err);
            anyhow!("{}", err)
        })?;
...
pub async fn rrai_cloud_post_multipart_file_stream(
    path: &String,
    token: &String,
    request_form: MultipartRequest<FileStream>,
) -> Result<String> {
    //
    let https = HttpsConnector::new();
    //构建Https Client
