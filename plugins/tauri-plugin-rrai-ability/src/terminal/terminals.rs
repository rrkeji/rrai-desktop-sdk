...
lazy_static::lazy_static! {
    pub(crate) static ref CURRENT_RUNNING_TERMINALS: RwLock<HashMap<String, Arc<Mutex<LocalProcessTerminal>>>> =
...
pub struct LocalProcessTerminal {
    pub id: String,
...
pub async fn async_execute_command(command: &String) -> Result<String> {
    ...
    //新建
    let task = LocalProcessTerminal::new(command)?;
    let task_id = task.id.clone();

    //
    let arc_task = Arc::new(Mutex::new(task));

    let mut cache = CURRENT_RUNNING_TERMINALS
        .write()
...
pub async fn terminal_stdout(task_id: &String) -> Result<Vec<String>> {
    tracing::debug!("running_command_stdout方法调用:{}", task_id);

    //判断是否已经打开
    let exist = CURRENT_RUNNING_TERMINALS
        .read()
        .map_err(|err| anyhow!("{}", err))?
    ...
    if exist {
        if let Some(arc_task) = CURRENT_RUNNING_TERMINALS
            .read()
            .map_err(|err| anyhow!("{}", err))?
            .get(task_id)
        {
...
pub async fn terminal_stderr(task_id: &String) -> Result<Vec<String>> {
    tracing::debug!("running_command_stdout方法调用:{}", task_id);
...
pub async fn terminal_command_status(task_id: &String, exit_remove: bool) -> Result<(bool, i32)> {
    tracing::debug!("running_command_stdout方法调用:{}", task_id);
